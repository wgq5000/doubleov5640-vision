...
u8 ov5640_init(u8 cmos_ch,u16 cmos_h_pixel,  u16 cmos_v_pixel,  u16 total_h_pixel, u16 total_v_pixel )
{
	...
	usleep(20000);  //OV5640上电20ms后才能进行配置

    //读取OV5640摄像头ID
    cam_id  = sccb_read_reg16(cmos_ch,0x300b);       //LSB  0x40
    cam_id |= sccb_read_reg16(cmos_ch,0x300a) << 8;  //MSB  ox56

    if(cam_id != 0x5640)  //获取到正确的OV5640 ID，开始配置相关参数
        return 1;
    else{
		//先对寄存器进行软件复位，使寄存器恢复初始值
		//寄存器软件复位后，需要延时1ms才能配置其它寄存器
		sccb_write_reg16(cmos_ch,0x3008,0x82); //Bit[7]:复位 Bit[6]:电源休眠

		//延时1ms
		usleep(1000);

		sccb_write_reg16(cmos_ch,0x3008,0x02); //正常工作模式
		sccb_write_reg16(cmos_ch,0x3103,0x02); //Bit[1]:1 PLL Clock
		//引脚输入/输出控制 FREX/VSYNC/HREF/PCLK/D[9:6]
		sccb_write_reg16(cmos_ch,0x3017,0xff);
		//引脚输入/输出控制 D[5:0]/GPIO1/GPIO0
		sccb_write_reg16(cmos_ch,0x3018,0xff);
		sccb_write_reg16(cmos_ch,0x3037,0x13); //PLL分频控制
		sccb_write_reg16(cmos_ch,0x3108,0x01); //系统根分频器
		...
		sccb_write_reg16(cmos_ch,0x3600,0x08); //VCM控制,用于自动聚焦
		sccb_write_reg16(cmos_ch,0x3601,0x33); //VCM控制,用于自动聚焦
		sccb_write_reg16(cmos_ch,0x302d,0x60); //系统控制
		sccb_write_reg16(cmos_ch,0x3620,0x52);
		sccb_write_reg16(cmos_ch,0x371b,0x20);
		sccb_write_reg16(cmos_ch,0x471c,0x50);
		sccb_write_reg16(cmos_ch,0x3a13,0x43); //AEC(自动曝光控制)
		sccb_write_reg16(cmos_ch,0x3a18,0x00); //AEC 增益上限
		sccb_write_reg16(cmos_ch,0x3a19,0xf8); //AEC 增益上限
		...
		sccb_write_reg16(cmos_ch,0x3c06,0x00); //light meter 1 阈值[15:8]
		sccb_write_reg16(cmos_ch,0x3c07,0x08); //light meter 1 阈值[7:0]
		sccb_write_reg16(cmos_ch,0x3c08,0x00); //light meter 2 阈值[15:8]
		sccb_write_reg16(cmos_ch,0x3c09,0x1c); //light meter 2 阈值[7:0]
		...
		sccb_write_reg16(cmos_ch,0x4001,0x02); //BLC(黑电平校准)补偿起始行号
		sccb_write_reg16(cmos_ch,0x4005,0x1a); //BLC(黑电平校准)补偿始终更新
		sccb_write_reg16(cmos_ch,0x3000,0x00); //系统块复位控制
		sccb_write_reg16(cmos_ch,0x3004,0xff); //时钟使能控制
		sccb_write_reg16(cmos_ch,0x4300,0x61); //格式控制 RGB565
		sccb_write_reg16(cmos_ch,0x501f,0x01); //ISP RGB
		sccb_write_reg16(cmos_ch,0x440e,0x00);
		sccb_write_reg16(cmos_ch,0x5000,0xa7); //ISP控制
		sccb_write_reg16(cmos_ch,0x3a0f,0x30); //AEC控制;stable range in high
		sccb_write_reg16(cmos_ch,0x3a10,0x28); //AEC控制;stable range in low
		sccb_write_reg16(cmos_ch,0x3a1b,0x30); //AEC控制;stable range out high
		sccb_write_reg16(cmos_ch,0x3a1e,0x26); //AEC控制;stable range out low
		sccb_write_reg16(cmos_ch,0x3a11,0x60); //AEC控制; fast zone high
		sccb_write_reg16(cmos_ch,0x3a1f,0x14); //AEC控制; fast zone low
		...
		//Gamma(伽马)控制cmos_ch, 0x5480~0x5490
...
//		sccb_write_reg16(cmos_ch,0x3036,0x3c); //PLL倍频  //48M
		sccb_write_reg16(cmos_ch,0x3036,0x5A); //PLL倍频  //72M
		...
		//设置输出像素个数
		sccb_write_reg16(cmos_ch,0x3808, cmos_h_pixel >> 8     );  //DVP 输出水平像素点数高4位
		sccb_write_reg16(cmos_ch,0x3809, cmos_h_pixel & 0x00FF );  //DVP 输出水平像素点数低8位
		sccb_write_reg16(cmos_ch,0x380a, cmos_v_pixel >> 8     );  //DVP 输出垂直像素点数高3位
		sccb_write_reg16(cmos_ch,0x380b, cmos_v_pixel & 0x00FF );  //DVP 输出垂直像素点数低8位
		sccb_write_reg16(cmos_ch,0x380c, total_h_pixel >> 8    );  //水平总像素大小高5位
		sccb_write_reg16(cmos_ch,0x380d, total_h_pixel & 0x00FF);  //水平总像素大小低8位
		sccb_write_reg16(cmos_ch,0x380e, total_v_pixel >> 8    );  //垂直总像素大小高5位
		sccb_write_reg16(cmos_ch,0x380f, total_v_pixel & 0x00FF);  //垂直总像素大小低8位
		...
		sccb_write_reg16(cmos_ch,0x4004,0x02); //BLC(背光) 2 lines
		sccb_write_reg16(cmos_ch,0x4713,0x03); //JPEG mode 3
		sccb_write_reg16(cmos_ch,0x4407,0x04); //量化标度
		...
		sccb_write_reg16(cmos_ch,0x5001,0xa3); //ISP 控制
		sccb_write_reg16(cmos_ch,0x3b07,0x0a); //帧曝光模式
		//彩条测试使能
		sccb_write_reg16(cmos_ch,0x503d,0x00); //0x00:正常模式 0x80:彩条显示
		//测试闪光灯功能
		sccb_write_reg16(cmos_ch,0x3016,0x02);
		sccb_write_reg16(cmos_ch,0x301c,0x02);
		sccb_write_reg16(cmos_ch,0x3019,0x02); //打开闪光灯
		sccb_write_reg16(cmos_ch,0x3019,0x00); //关闭闪光灯
		return 0;
	}
